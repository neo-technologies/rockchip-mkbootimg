...
void usage(const char *appname) {
	...
	printf("USAGE:\n"
			...
			"%s -rk322a Loader.bin 5 0 0 rawimage.img rkimage.img \tRK322x board\n"
			"\n\n"
			"Options:\n"
			"[chiptype]:\n\t-rk29\n\t-rk30\n\t-rk31\n\t-rk3128\n\t-rk32\n\t-rk3368\n\t-rk322a\n", p, p, p, p, p, p, p);
}
...
int main(int argc, char **argv)
{
	...
	if (argc == 8)
	{
		...
		else if (strcmp(argv[1], "-rk322a") == 0)
		{
			ret = pack_rom(0x33323241, argv[2], atoi(argv[3]), atoi(argv[4]), atoi(argv[5]), argv[6], argv[7]);
		}
		else
		{
			usage(argv[0]);
